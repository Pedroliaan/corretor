...
class Questao:
    '''Uma questao para corrigir.'''

    def __init__(self, descricao: str, comando: str, script: str, correcoes: list['Correcao']):
        '''Construtor.

        Parametros:
        - `descricao` e uma descricao da questao.
        - `comando` e o comando do terminal para executar o script da resposta.
        - `script` e o script da resposta.
        - `correcoes` sao os argumentos e verificacões da saida do script para corrigir a questao.
        '''
    ...
    def ler_config(cls, config_questao: dict) -> 'Questao':
        '''Cria uma instancia a partir do dict obtido da leitura do arquivo config.json.
        
        Parametros:
        - `config_questao` e o dict de uma questao (um elemento da lista "questoes").'''
...
class Correcao:
    ...
    def __init__(self, comando: str, script: str, input_: str, cli_args: str, func_expect, args_expect: list):
        ...
        - `comando` e o comando do terminal para executar o script da resposta.
        - `script` e o script da resposta.
        - `input_` e a entrada do teclado.
        - `cli_args` sao os argumentos da linha de comando salvos no atributo `args`.
        - `func_expect` e a funcao que verifica a saida do script.
        - `args_expect` sao os argumentos da funcao que verifica a saida do script.'''
    ...
    def ler_config(cls, comando: str, script: str, config_correcao: dict) -> 'Correcao':
        ...
        Parametros:
        - `comando` e `script` sao os mesmos da `Questao`.
        - `config_correcao` e o dict de uma correcao (um elemento da lista "correcoes").'''
    ...
    def corrigir(self) -> tuple[int, str, str]:
        '''Executa a correcao e retorna o codigo de saida, a saida e o erro.'''
        codigo = -1
        resposta = 'Nao executado\n'
        erro = 'Nao executado\n'
        try:
...
def testar_igual(resultado: str, esperado: str) -> tuple[bool, str]:
    '''Verifica se o `resultado` e igual ao `esperado`.'''
...
def testar_regex(resultado: str, regex: str) -> tuple[bool, str]:
    ...
    if padrao.search(resultado) is None:
        erro = f'Esperava encontrar o padrao "{regex}".'
        return False, erro
    return True, ''
...
class ScrolledFrame(ttk.Frame):
    '''Frame com scrollbar.
    *ATENCAO:* para colocar widgets dentro deste, passe o `.conteudo` deste como `parent` do widget filho.'''
    def __init__(self, parent, width, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        # Na raiz, e necessario um Canvas
        ...
        # Dentro do canvas, e necessario um Frame
    ...
    def _on_resize(self, event):
        ...
        # bbox e uma tupla (x, y, largura, altura) que engloba todo o conteúdo do canvas
        bbox = self.canvas.bbox('all')
        # Existe algum problema que ela pega alem do tamanho do que e visivel,
        # entao consertamos isso copiando os demais valores e recalculando a altura
...
class Corretor():
    ...
    def __init__(self, caminho_config: str):
        '''Construtor.
        `caminho_config` e o caminho para o arquivo json de configuracao da correcao.'''
        super().__init__()
        janela = tk.Tk()
        self.janela = janela
        ...
        # Lê o arquivo de configuracao
        self.config = json.load(open(caminho_config))
        # Configura a janela
        janela.title(f"Corretor Automatico - {self.config['titulo']}")
        janela.geometry("1024x600")

        # Montagem da interface
        # O frame principal contem todos os elementos da tela
...
class QuestaoWidget(ttk.Frame):
    ...
    def __init__(self, parent, janela_corretor: Corretor, questao: Questao):
        '''Construtor.
        Parametros:
        - `parent` e o widget pai que contera este.
        - `questao` e a questao correspondente.
        - `janela_corretor` e uma referência à aplicacao do corretor.'''
    ...
    def _montar_primeira_linha(self):
        '''Monta a primeira linha deste widget, que contem a descricao da questao, o botao para corrigir e o label do resultado.'''
        ...
        self.botao_corrigir = ttk.Button(frame2, text=' Corrigir Questao',
    ...
    def _montar_correcoes(self):
        '''Monta o widget de cada correcao.'''
    ...
    def _corrigir_questao(self):
        '''Executa todas as correcoes da questao.'''
    ...
    def correta(self) -> bool:
        '''Retorna True se todas as correcões estao corretas e False, caso contrario.'''
        return self.contador_corretas == len(self.widgets_correcoes)


class CorrecaoWidget(ttk.Frame):
    '''Widget da Correcao.'''
    resultado = 'Nao executada'

    def __init__(self, parent, correcao: Correcao):
        '''Construtor.
        Parametros:
        - `parent` e o widget pai que contera este.
        - `correcao` e a correcao correspondente.'''
        super().__init__(parent)
    ...
    def _corrigir(self):
        ...
        res = ''  # Guarda o resultado da correcao
        if saida:
            saida = saida  # Remove a linha extra que sempre vem
            res += f'Saida:\n{saida}'
        if erro:
            res += f'Erro:\n{erro}'
        text.configure(state=tk.NORMAL)  # Habilita a caixa de texto para edicao
        ...
        text.configure(height=altura,
                       state=tk.DISABLED)  # Desabilita a edicao
        # Atualiza o label do resultado e a cor do botao
        estilo = 'TButton' # Comeca com botao comum
        # Mesmo o codigo sendo 0, a saida precisa ser a esperada, entao e necessario que erro seja None
        if erro:
